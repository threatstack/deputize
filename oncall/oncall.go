...
func UpdateOnCallRotation(conf config.DeputizeConfig) error {
	...
	if err != nil {
		return fmt.Errorf("error initializing Vault client: %s", err)
	}
	if conf.TokenPath == "" {
	...
	getSecret, err := vaultClient.Logical().Read(conf.VaultSecretPath)
	if err != nil {
		return fmt.Errorf("Unable to read secrets from vault: %s", conf.VaultSecretPath)
	}

	secret, ok := getSecret.Data["data"].(map[string]interface{})
	if !ok {
		return fmt.Errorf("Unable to map vault's data element to a map[string]interface")
	}

	// Begin talking to PagerDuty
	client := pagerduty.NewClient(secret["pdAuthToken"].(string))
	if !conf.Quiet {
	...
	if reflect.DeepEqual(currentOnCallUids, newOnCallUids) {
		...
		if err := l.Bind(conf.ModUserDN, secret["modUserPW"].(string)); err != nil {
		...
		if conf.GitlabEnabled {
			...
			gitlabClient := gitlab.NewClient(nil, secret["gitlabAuthToken"].(string))
			...
			for _, email := range newOnCallApproverEmails {
				...
				if len(users) == 1 {
					...
					if !conf.Quiet {
						log.Printf("No user found for email %s (%s)\n", users[0].Username, email)
					}
				} else {
		...
		if conf.SlackEnabled == true {
			slackAPI := slack.New(secret["slackAuthToken"].(string))
